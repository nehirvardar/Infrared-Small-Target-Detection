...
class Light_ASPP(nn.Module):
    def __init__(self, channels):
        super(Light_ASPP, self).__init__()
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(channels, channels // 2, 3, 1, padding=3, dilation=3, bias=False),
            nn.BatchNorm2d(channels // 2),
            nn.ReLU(inplace=True)
        )
        self.branch3 = nn.Sequential(
            nn.Conv2d(channels, channels // 2, 3, 1, padding=5, dilation=5, bias=False),
            nn.BatchNorm2d(channels // 2),
            nn.ReLU(inplace=True)
        )
        self.conv_out = nn.Sequential(
            nn.Conv2d((channels // 2) * 3, channels, 1, 1, 0, bias=False),
...
class DNANet_vers5(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks, nb_filter, deep_supervision=False):
        ...
        # ---> YENİ: Light-ASPP Modülünün Tanımlanması <---
        self.light_aspp = Light_ASPP(nb_filter[4])
    ...
    def forward(self, input):
        ...
        # ---> YENİ: x4_0 Darboğazını (Bottleneck) Light-ASPP'den geçir <---
        x4_0 = self.light_aspp(x4_0)
